...
public class EnergySync {
    ...
    public boolean handle(Supplier<NetworkEvent.Context> supplier){
        ...
        context.enqueueWork(() -> {
            ...
            if (Minecraft.getInstance().level.getBlockEntity(pos) instanceof CulinaryGeneratorBlockEntity blockEntity){
                blockEntity.setEnergyLevel(energy);

                if (Minecraft.getInstance().player.containerMenu instanceof CulinaryGeneratorMenu menu && menu.getBlockEntity().getBlockPos().equals(pos)){
                    blockEntity.setEnergyLevel(energy);
                }
            }
        });
